...
public class PasseioController {
    ...
    @GetMapping
    public List<Passeio> listar(@RequestAttribute("usuario") UsuarioLogado usuario) {
        return service.listar(usuario);
    }

    @GetMapping("/{id}")
    public Passeio buscar(@RequestAttribute("usuario") UsuarioLogado usuario, @PathVariable String id) {
        return service.buscar(usuario, id);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public Passeio criar(@RequestAttribute("usuario") UsuarioLogado usuario, @RequestBody PasseioRequest pedido) {
        return service.criar(usuario, pedido);
    }

    @PutMapping("/{id}")
    public Passeio atualizar(
            @RequestAttribute("usuario") UsuarioLogado usuario,
            @PathVariable String id,
            @RequestBody PasseioRequest pedido
    ) {
