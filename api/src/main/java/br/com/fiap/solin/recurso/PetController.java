...
public class PetController {
    ...
    @GetMapping
    public List<Pet> listar(@RequestAttribute("usuario") UsuarioLogado usuario) {
        return service.listar(usuario);
    }

    @GetMapping("/{id}")
    public Pet buscar(@RequestAttribute("usuario") UsuarioLogado usuario, @PathVariable String id) {
        return service.buscar(usuario, id);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public Pet criar(@RequestAttribute("usuario") UsuarioLogado usuario, @RequestBody PetRequest pedido) {
